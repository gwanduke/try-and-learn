...
- 😎 this는 몇가지로 추론될 수 있는가?
  - 전역범위에서 사용될때 this는 `전역객체`를 가르킨다. 함수에서 사용될때도 `전역객체`를 가르킴. 객체에 속한 메서스에서 사용될때 그 `메서드의 객체`를 가르킨다. 객체에 속한 메서드의 내부함수에서 사용될땐 `전역객체`를 가르킨다. 생성자에서 사용될때 생성자로 인해 생성된 `새로운 객체`를 가리킨다.
